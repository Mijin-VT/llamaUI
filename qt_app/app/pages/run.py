...
class RunPage(PageBase):
    ...
    def _build_runtime_header(self) -> None:
        ...
        self._save_button = SuccessButton("Save Profile", hero); self._save_button.clicked.connect(self._save_profile)
        self._save_as_button = SecondaryButton("Save As", hero); self._save_as_button.clicked.connect(self._save_profile_as)
        self._duplicate_button = SecondaryButton("Duplicate", hero); self._duplicate_button.clicked.connect(self._duplicate_profile)
        self._reset_button = DangerButton("Reset", hero); self._reset_button.clicked.connect(self._reset_form_to_profile)
        ...
        self._apply_preset_button = SecondaryButton("Apply Preset", hero); self._apply_preset_button.clicked.connect(self._apply_preset_from_combo)
        self._reset_defaults_button = SecondaryButton("Reset to defaults", hero); self._reset_defaults_button.clicked.connect(self._reset_to_defaults)
        self._start_button = SuccessButton("Start", hero); self._start_button.clicked.connect(self._start)
        self._stop_button = DangerButton("Stop", hero); self._stop_button.clicked.connect(self._stop)
        self._restart_button = SecondaryButton("Restart", hero); self._restart_button.clicked.connect(self._restart)
        self._switch_button = SecondaryButton("Load via API / Restart fallback", hero); self._switch_button.clicked.connect(self._switch_model)
        ...
        for widget in (self._start_button, self._stop_button, self._restart_button, self._switch_button):
        ...
        for widget in (self._save_button, self._save_as_button, self._duplicate_button, self._reset_button, self.preset_combo, self._apply_preset_button, self._reset_defaults_button):
    ...
    def _auto_populate_mmproj(self) -> None:
        ...
        if editor is not None:
            if model and model.mmproj_path:
                ...
                self.status.setText(f"Auto-detectado proyector visión: {Path(model.mmproj_path).name}")
            else:
                editor.clear()
                profile = self._selected_profile()
        ...
        # Auto-parse .bin parameter config file from the model's directory
        if model and getattr(model, "bin_config_path", None):
            bin_path = Path(model.bin_config_path)
            if bin_path.exists():
                from ..services.library_scan import parse_bin_config
                parsed = parse_bin_config(bin_path)
                if parsed:
                    profile = self._selected_profile()
                    for opt_id, val in parsed.items():
                        catalog_opt = LLAMA_OPTION_CATALOG.get(opt_id)
                        opt_widget = self._editors.get(opt_id)
                        if catalog_opt and opt_widget:
                            self._set_editor_value(catalog_opt, opt_widget, val)
                            if profile is not None:
                                from llama_data.models import OptionValue
                                profile.settings[opt_id] = OptionValue(val)
                                profile.user_set.add(opt_id)
                    if profile is not None:
                        self.profile_store.upsert(profile)
                    self.status.setText(f"Auto-configurados parámetros desde {bin_path.name}")
    ...
    def _start(self) -> None:
        try:
            ...
            if not config.llama_server_path or not Path(config.llama_server_path).is_file():
                from ..services.runtime_api import find_llama_server_binary
                found = find_llama_server_binary()
                if found:
                    config.llama_server_path = str(found)
                    self.config_store.save(config)
                    self.status.setText(f"Auto-detectado ejecutable llama-server en: {found.name}")
                else:
                    reply = QMessageBox.question(
                        self,
                        "llama-server.exe no configurado",
                        "No se ha configurado la ruta del ejecutable 'llama-server.exe'.\n\n"
                        "¿Deseas buscar y seleccionar el archivo 'llama-server.exe' en tu equipo ahora?",
                        QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                    )
                    if reply == QMessageBox.StandardButton.Yes:
                        selected, _ = QFileDialog.getOpenFileName(
                            self,
                            "Seleccionar ejecutable llama-server.exe",
                            str(Path.home()),
                            "Ejecutables (*.exe llama-server*);;Todos los archivos (*.*)",
                        )
                        if selected:
                            config.llama_server_path = str(Path(selected).resolve())
                            self.config_store.save(config)
                        else:
                            self.status.setText("Inicio cancelado: falta llama-server.exe")
                            return
                    else:
                        self.status.setText("Inicio cancelado: configura llama-server.exe en Settings")
                        return
    ...
    def _update_controls_lock(self, is_running: bool) -> None:
        """Lock (disable) all parameter editors and profile configuration controls
        when the server is running/starting, and unlock (enable) them when stopped.
        """
        if hasattr(self, "_start_button") and self._start_button:
            self._start_button.setEnabled(not is_running)

        if hasattr(self, "_stop_button") and self._stop_button:
            self._stop_button.setEnabled(is_running)

        if hasattr(self, "_mode_combo") and self._mode_combo:
        ...
        if hasattr(self, "profile_combo") and self.profile_combo:
            self.profile_combo.setEnabled(not is_running)

        for btn_attr in ("_save_button", "_save_as_button", "_duplicate_button", "_reset_button",
                          "preset_combo", "_apply_preset_button", "_reset_defaults_button"):
    ...
    def _set_status(self, status) -> None:
        from ..services.runtime_api import ServerState
        is_active = status.state in {ServerState.STARTING, ServerState.RUNNING, ServerState.HEALTHY, ServerState.UNHEALTHY}
        self._update_controls_lock(is_active)
