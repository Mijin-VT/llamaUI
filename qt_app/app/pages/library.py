...
from ..widgets.scan_dialogs import ScanCustomFolderOptionsDialog, ScanProgressDialog
...
class LibraryPage(PageBase):
    ...
    def _build_header(self) -> None:
        ...
        self._rescan = QPushButton("🔄 Rescan", controls)
        ...
        self._custom_scan_btn = SuccessButton("📂 Scan Custom Folder…", controls)
        self._custom_scan_btn.setToolTip("Escanear cualquier carpeta del sistema en busca de modelos .gguf.")
        self._custom_scan_btn.clicked.connect(self._on_scan_custom_folder)
        controls_layout.addWidget(self._custom_scan_btn)
        ...
        # Three-dots menu (⋮)
        self._more_menu_btn = QToolButton(controls)
        self._more_menu_btn.setText("⋮")
        self._more_menu_btn.setToolTip("Más opciones de escaneo")
        self._more_menu_btn.setPopupMode(QToolButton.ToolButtonPopupMode.InstantPopup)
        self._more_menu_btn.setStyleSheet(
            f"QToolButton {{ background-color: {theme.BG_RAISED}; color: {theme.FG_PRIMARY}; border: 1px solid {theme.BORDER}; border-radius: 6px; padding: 4px 8px; font-weight: bold; font-size: 14px; }} "
            f"QToolButton:hover {{ background-color: {theme.BORDER_HOVER}; }}"
        )
        self._more_menu = QMenu(self._more_menu_btn)
        act_custom = self._more_menu.addAction("📂 Scan Custom Folder...")
        act_custom.triggered.connect(self._on_scan_custom_folder)
        act_rescan = self._more_menu.addAction("🔄 Rescan Models Directory")
        act_rescan.triggered.connect(self._on_rescan)
        self._more_menu_btn.setMenu(self._more_menu)
        controls_layout.addWidget(self._more_menu_btn)
    ...
    def deferred_refresh(self) -> None:
        """Fast visual refresh from store without blocking disk scan."""
        self._refresh()
    ...
    def _on_rescan(self) -> None:
        ...
        models_dir_path = Path(cfg.models_dir).expanduser() if cfg.models_dir else None

        if not models_dir_path or not models_dir_path.is_dir():
            reply = QMessageBox.question(
                self,
                "Carpeta de modelos no configurada",
                "No se ha configurado una carpeta de modelos principal en la aplicación (o la ruta configurada no existe).\n\n"
                "¿Deseas seleccionar tu carpeta principal de modelos .gguf ahora?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if reply == QMessageBox.StandardButton.Yes:
                selected = QFileDialog.getExistingDirectory(
                    self, "Seleccionar carpeta principal de modelos", str(Path.home())
                )
                if selected:
                    cfg.models_dir = str(Path(selected).resolve())
        ...
        if result.error:
            ...
            self._detail_title.setText(f"Escaneo fallido: {result.error}")
            QMessageBox.warning(self, "Error de escaneo", f"No se pudo completar el escaneo:\n{result.error}")
        else:
            self._rescan.setText(f"+{result.added} nuevos")
            msg = (
                f"Escaneo de carpeta principal completado.\n\n"
                f"• Nuevos modelos encontrados: {result.added}\n"
                f"• Modelos actualizados: {result.updated}\n"
                f"• Modelos obsoletos removidos: {result.removed}\n"
                f"• Total archivos procesados: {result.scanned_files}"
            )
            QMessageBox.information(self, "Resumen del escaneo", msg)
        self._refresh()
        QTimer.singleShot(2500, lambda: self._rescan.setText("🔄 Rescan"))



    def _on_scan_custom_folder(self) -> None:
        cfg = self._config_store.load()
        default_dir = Path(cfg.models_dir) if cfg.models_dir and Path(cfg.models_dir).is_dir() else Path.home()
        ...
        options_dialog = ScanCustomFolderOptionsDialog(Path(initial_dir), parent=self)
        if options_dialog.exec() != QDialog.DialogCode.Accepted:
            return

        options = options_dialog.get_options()

        progress_dialog = ScanProgressDialog(options, self._library_store, parent=self)
        result_code = progress_dialog.exec()

        if result_code == QDialog.DialogCode.Accepted and progress_dialog.result:
            res = progress_dialog.result
            if res.error:
                QMessageBox.warning(self, "Error de escaneo", f"El escaneo falló: {res.error}")
            else:
                msg = (
                    f"Escaneo completado exitosamente.\n\n"
                    f"• Se agregaron {res.added} modelos nuevos.\n"
                    f"• Se actualizaron {res.updated} modelos.\n"
                    f"• Total archivos escaneados: {res.scanned_files}"
                )
                QMessageBox.information(self, "Resumen del escaneo", msg)
                self._refresh()
