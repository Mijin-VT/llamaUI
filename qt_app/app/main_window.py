...
from llama_data import ChatStore, ConfigStore, LibraryStore, ProfileStore
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        # Auto-detect llama-server binary if not configured yet
        try:
            cfg = config_store.load()
            if not cfg.llama_server_path or not Path(cfg.llama_server_path).is_file():
                from .services.runtime_api import find_llama_server_binary
                found = find_llama_server_binary()
                if found:
                    cfg.llama_server_path = str(found)
                    config_store.save(cfg)
        except Exception:
            pass

        self._pages: dict[NavItemId, QWidget] = {
            NavItemId.CHAT: ChatPage(chat_store=chat_store, config_store=config_store),
    ...
    def navigate(self, item_id: NavItemId) -> None:
        ...
        # Defer refresh to allow Qt to paint tab transition first (0ms lag)
        def _deferred():
            if hasattr(page, "deferred_refresh"):
                page.deferred_refresh()
