...
def _first_mmproj(companion_paths: list[str]) -> Optional[str]:
    for p in companion_paths:
        ...
        if name.startswith("mmproj-") or "mmproj" in name or "vision" in name:
            if p.lower().endswith(".gguf") or p.lower().endswith(".bin"):
                return p
    return None


def _first_bin_config(companion_paths: list[str]) -> Optional[str]:
    for p in companion_paths:
        name = Path(p).name.lower()
        if p.lower().endswith(".bin") and not name.startswith("mmproj-"):
            return p
    return None
...
class LocalModel:
    ...
    def to_json(self) -> dict[str, Any]:
        ...
        if d.get("bin_config_path") is None:
            d.pop("bin_config_path", None)
        return d
    ...
    def from_json(cls, data: Any) -> "LocalModel":
        ...
        if "bin_config_path" not in cleaned or cleaned.get("bin_config_path") is None:
            cleaned["bin_config_path"] = _first_bin_config(cleaned.get("companion_paths", []))
        return cls(**cleaned)
