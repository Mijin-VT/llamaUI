...
print(f"PASS: Discover page inspector width = {_inspector_width()}")

# -- Test 4: navigate to Profiles (STANDARD) --------------------------------
win.navigate(NavItemId.PROFILES)
assert _inspector_width() >= theme.INSPECTOR_MIN_WIDTH, (
    f"PROFILES page (STANDARD) should show inspector, got width={_inspector_width()}"
)
print(f"PASS: Profiles page inspector width = {_inspector_width()}")

# -- Test 5: navigate to Settings (INSPECTOR_OPTIONAL → collapsed) ----------
...
# -- Test 6: navigate to Diagnostics (FULL_WIDTH → hidden) ------------------
win.navigate(NavItemId.DIAGNOSTICS)
assert not win.inspector.isVisible(), (
    "DIAGNOSTICS page (FULL_WIDTH) should hide the inspector widget"
)
assert _inspector_width() == 0, (
...
# -- Test 7: navigate back to RUN → inspector restored ----------------------
...
# -- Test 8: set_inspector_visible toggles correctly ------------------------
...
# -- Test 9: sidebar uses minimum width, not fixed --------------------------
sw = win.sidebar
...
# -- Test 10: inspector uses minimum width, not fixed -----------------------
iw = win.inspector
...
from app.pages.profiles import ProfilesPage  # noqa: E402
...
assert ProfilesPage.policy == PagePolicy.STANDARD
